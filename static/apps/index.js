...
function createContext(context, words, i, n) {
    ...
    if (n > 0) createContext(context[words[i + 1]].context, words, i + 1, n - 1);
};
...
function getWords(string) {
    //a word is a space delimited string contained in another string
    //console.log(string);
...
function genProb(D) {
    ...
    */
    ...
    if ((typeof D === "undefined" ? "undefined" : _typeof(D)) !== "object") throw new Error("expected an object");
    var P = [],
        T = [],
        R = [],
        A = {},
    ...
    if (keys.length > 0) {
        ...
        return {
            ...
            updateProb: function updateProb(d) {
                Object.keys(d).forEach(function (word, i) {
                    if (!A[word]) {
                        A[word] = i;
                        P.push(d.context[word].count);
                        T.push(word);
                        ...
                        if (d.depth > 0) {
                            R[A[word]].updateProb(d.context[word]);
                        }
                    }
                });
                return this;
