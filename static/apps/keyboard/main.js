...
function saveSong(songName,cb) {
    socket.emit("saveJSON","./songs/"+songName+".json",songlang,function(dat) {
        if(typeof cb === "function")cb(dat);
        console.log("response for saveJSON recieved",dat);
    });
}
function loadSong(songName,cb) {
    socket.emit("loadJSON","./songs/"+songName+".json",function(data) {
        songlang = data;
        if(typeof cb === "function")cb(data);
        console.log("response for readJSON recieved",arguments);
    });
}
...
function generateLanguageNieve() {
    ...
    songlang = {
        velocity:genProb(createDictionary(lang.velocity,15)),
        note:genProb(createDictionary(lang.note,15)),
        delay:genProb(createDictionary(lang.delay,15)),
        time:genProb(createDictionary(lang.time,15)),
    }
